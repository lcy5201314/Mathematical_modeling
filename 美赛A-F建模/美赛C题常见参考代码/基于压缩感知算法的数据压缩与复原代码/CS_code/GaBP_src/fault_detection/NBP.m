...
function [xrecon, mrecon, srecon, iter, convergence]=NBP(A,x,y,sigma_Z,max_iter,...
    dispind,epsilon,pdf_prior,noise_prior,xticks,show, del)
...
last_norm = inf;
mrecon_c = zeros(1,n);
Delta = del; %this is the change in norm under which we stop - it can be large when discretization is mandatory
conv = zeros(n,max_iter);
...
for it=1:max_iter	
   ...
   for in=1:n
      ...
      if (it==1) % initial round - send the signal prior
         ...
         [mrecon_c(in), srecon(in),xrecon(in)]=meanvarmaxpdf((pdf_res), xticks); % computes statistics
   ...
   if (it >=2) % display and break on last iteration
      if show
          [flag,last_norm] = dispvec_anderrors_gabp(mrecon_c, A, y, dispind, x, last_norm);
      end
      conv(:,it-1) = mrecon_c;
      if norm(mrecon-mrecon_c)<Delta
          iter = it;
          break;
      end
      if (it==max_iter)
...
function [npdf]= pdf_integral(pdf, model_order, edge,xticks,delta,epsilon)
   ...
   elseif  (edge == 1)
       ...
       npdf= interp1(xticks, pdf, xticks./abs(edge));%,'pchip');
