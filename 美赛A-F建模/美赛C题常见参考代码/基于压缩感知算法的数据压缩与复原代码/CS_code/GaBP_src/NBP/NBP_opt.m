...
function [xrecon, mrecon, srecon]=NBP_opt(A,x,y,sigma_Z,max_iter,...
...
global offsets;
offsets= model_order:-1:2;

if (max(abs(y)) >= max(abs(xticks)))
	disp(['overflow in y value: ' , num2str(max(abs(y))), ' xticks is ', num2str(max(abs(xticks)))]);
...
for it=1:max_iter	
   ...
   for in=1:m
      ...
      for j=1:ln   %To send next message
         ...
         pdf_res=shiftpdf(pdf_res, y(in), delta, xticks, model_order);  
         %%%assert(sum(isnan(pdf_res))==0);
...
function pdf=shiftpdf(pdf, offset, delta, xx, model_order)
    ...
    pdf2=pdf;
    %pdf1 = interp1(xx,pdf,xx-offset);
    howmuch = round(offset/delta);
    %pdf=ifftshift(pdf);
    %if (howmuch > 0)
    %    pdf = [ 1e-70*ones(1,howmuch) pdf(1:model_order-howmuch) ];
    %elseif (howmuch < 0)
    %    pdf = [ pdf(-howmuch+1:model_order) 1e-70*ones(1,-howmuch)];
    %end
    %pdf=fftshift(pdf);
    if (howmuch > 0)
       pdf2 = [ pdf2(end-howmuch+1:end) pdf2(1:(model_order-1)/2-howmuch) 1e-70*ones(1,howmuch) pdf2((model_order+1)/2:end-howmuch)];
    elseif (howmuch < 0)
        pdf2 = [ pdf2(-howmuch+1:(model_order-1)/2) 1e-70*ones(1,-howmuch)  pdf2((model_order-1)/2-howmuch+1:end) pdf2(1:-howmuch)];
    end
    %plot(xx,pdf,xx,pdf2);
    %pdf(isnan(pdf)) = 0;
    pdf = pdf2./sum(pdf2);
