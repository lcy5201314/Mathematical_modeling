...
function [xrecon, mrecon, srecon]=NBP2(A,x,y,sigma_Z,max_iter,...
...
for it=1:max_iter	
   ...
   for i=1:n
      ...
      if (it==1) % initial round - send the signal prior
         ...
         for j=1:ln   % to send next message
            m_ji = M2N{neighbors(j),i}+epsilon;
             m_ji = pdf_integral(m_ji, model_order, A(neighbors(j),i),xticks,delta,epsilon);
   ...
   for i=1:m
      ...
      for j=1:ln % process neighbors
         ...
         if (j == 1) % first time
            pdf_res_all=m_ij+epsilon;
         else           
            pdf_res_all=mulpdf(pdf_res_all, m_ij);
         end
      end
      
      %convolve with the self potential of the noise
      if (sigma_Z>epsilon)
         pdf_res_all=mulpdf(pdf_res_all, noise_prior);
      end
      ...
      for j=1:ln   %To send next message
         ...
         pdf_res=divpdf(pdf_res_all, m_ij);
