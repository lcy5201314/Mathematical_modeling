...
H=[0 0.2 0.3; 0.2 0 -0.2; 0.3 -0.2 0];
...
sigma = .001;
% 
y = H*transmit+randn(n,1)*sigma;


%y =ones(3,1);
% BIGy=[zeros(3,1)' y']';

 [x,p] = GBP(H+eye(3),y,10,0.00000001);
% x
% p
...
%x = NBP(H,self_pot,30, 1e-10,'exact')
 [meanx, varx, maxx,X]=UNBP(H,transmit,ones(3,1),self_pot,...
...
%     max_rounds, displayind ,epsilon, xx);
 ...
 fprintf('[   %d   %1d   %d   %d]\n', transmit(displayind));
 fprintf('success=%6.2f \n',sum((((meanx(1:n)>0)*2)-1)==transmit')/n);
% 
 fprintf('GaBP success=%6.2f  \n',sum(transmit == sign(inv(H)*y))/n);
