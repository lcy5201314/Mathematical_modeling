function [rec_img, psnr_val, num_nz_theta, Num_Iteration] = My_fast_cs2d(im, K, trans_mode, rand_type, blk_size, SI, Iteration)
%  ===== Required inputs ============= 
...
% Main loop to test the reconstruction performance for each K(i);
 for i =1:length(K),
   ...
   Bt = @(z) At_dwt1d(Phi_T,z,h0,h1,L,m,n);
    ...
    [alp, alp_debias, objective, times, debias_start, mses, num_nz_theta, Num_Iteration, psnr_val]= ...
    My_GPSR_BB(y, B, tau, 'AT', Bt, 'Initialization', dwt_sb, 'SI', dwt_sb, 'MaxiterA', Iteration, 'OriX', x0, 'OriSI', s0, 'Num_of_Measurements', K); 
    alp = reshape(alp, m, n);
    xr = idwt2d(alp, f0, f1, L);
