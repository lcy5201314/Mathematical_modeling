function [rec_img, psnr_val, num_nz_theta, Num_Iteration] = fast_cs2d(im, K, trans_mode, rand_type, blk_size, Iteration)
%  ===== Required inputs ============= 
...
% Main loop to test the reconstruction performance for each K(i);
 for i =1:length(K),
   ...
   Bt = @(z) At_dwt1d(Phi_T,z,h0,h1,L,m,n);
    ...
    [alp, objective, times, mses, num_nz_theta, Num_Iteration, psnr_val] = ...
    GPSR_BB(y, B, tau, 'AT', Bt, 'MaxiterA', Iteration, 'OriX', x0);
