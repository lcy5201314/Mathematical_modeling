...
explicitA = ~(ischar(A) || isa(A, 'function_handle'));%explicit A表示矩阵满秩矩阵
n = length(y);
...
while ~done
    ...
    if (explicitA)
        corr = sqrt(n) .* A'*res ./ norm(res);%res ./ norm(res)归一化
    else
    ...
    switch lower(thresh)
        case 'fdr'
            thr = fdrthresh(corr, q);%产生阈值参数
        case 'far'
            thr = norminv(1 - alpha_0/2, 0, 1);%参数阈值参数
    end

    % Apply hard thresholding
    I = find(abs(corr) > thr);%查找corr中非零项的索引
