...
while(norm(rn)>5)
    ...
    a=(rn'*cn)/norm(cn).^2;%步长
        ...
        a=-(gn'*d)/(d'*H*d);
%         hat_x(pos_array)=hat_x(pos_array)+a*d  ;             %hat_x(pos_array)=hat_x(pos_array)+d*gn
%         rn=rn-Aug_t* a*d;
       
       
    end
     hat_x(pos_array)=hat_x(pos_array)+a*d  ;
    
    rn=rn-a*Aug_t*d;                           %a*Aug_t*d
    T(:,pos)=zeros(M,1);
    times=times+1;
end
